...
```{r}
lithic_data <-
  read_csv(paste0(here::here(), "/data/lithic_data.csv"), 
           na = c("NA", '<NA>'))
str(lithic_data)
...
arc_data <- read_csv(paste0(here::here(), "/data/arc_data.csv"))
...
experiment_data <-
  read_csv(paste0(here::here(), "/data/experiment_data.csv"), 
           na = c("NA", "", '<NA>'))
...
# windows()
png_file <- paste0(here::here(), "/code/model_diagnostics_plot.png")
png(png_file, width = 500, height = 500, units = "px")
par(mfrow = c(2, 2))
...
```{r}
plot(series_2_shape_gamlss1) #plot model diagnostics
```
...
tool_event_coords_subset <- read_csv(paste0(here::here(), "/data/tool_three_event_movement.csv"))
...
```{r eval = FALSE}
# I set eval = FALSE to save time when knitting this repeatedly while working on other sections. 
